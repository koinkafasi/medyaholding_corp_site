...
export default function NewsContent() {
    ...
    return (
        <div className="bg-white dark:bg-[#070707] w-full px-[16px] md:px-[72px] py-[48px] md:py-[80px]">
            <Tag>
                <Typography size={14} sizeMobile={12} weight={500}>{dict.news.eyebrow}</Typography>
            </Tag>
            <div className="mt-[64px] bg-[#EEE] dark:bg-[#1D1D1D] rounded-[20px] p-[32px] md:p-[60px] flex flex-col items-start gap-[16px] max-w-[640px]">
                <Typography as="div" size={28} weight={700}>
                    {dict.news.title}
                </Typography>
                <Typography size={16} lineHeight={26} className="opacity-70">
                    {dict.news.pendingNote}
                </Typography>
            </div>
        </div>
    )
